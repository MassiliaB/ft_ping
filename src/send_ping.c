...
int send_packet(int msg_count, int raw_sockfd, struct sockaddr_in *ping_addr, t_params *global)
{
    struct icmp *icmp = (struct icmp *) global->s_packet;
    memset(global->s_packet, 0, DATALEN);
    ...
    if ((sendto(raw_sockfd, global->s_packet, DATALEN, 0, (struct sockaddr*)ping_addr, sizeof(struct sockaddr_in))) < 0) {
        printf("ping: ");
...
int recieve_packet(int raw_sockfd, t_params *global)
{
    ...
    memset(global->r_packet, 0, PACKET_SIZE);
    if ((len = recvfrom(raw_sockfd, global->r_packet, PACKET_SIZE, 0, (struct sockaddr*)&r_addr, &addr_len)) < 0){
...
void    icmp_loop(int raw_sockfd, struct sockaddr_in *ping_addr, char *argv, char *ip_addr, t_params *global)
{
