...
void    icmp_loop(int raw_sockfd, struct sockaddr_in *ping_addr, struct timespec *tfs, struct timespec *tfe, char *argv, char *ip_addr, int ttl_val, char *ping_domain)
{
    ...
    printf("here 5\n");
       ...
       // memset(r_buffer, 0, sizeof(r_buffer));
        bzero(&hdr_r_pckt, sizeof(hdr_r_pckt)); // filling packet

        // receive packet
        if (recvfrom(raw_sockfd, &hdr_r_pckt, sizeof(hdr_r_pckt), 0, (struct sockaddr*)&r_addr, (socklen_t*)(sizeof(r_addr))) < 0 && msg_count > 1) {
